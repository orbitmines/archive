...
User-controlled ownership of data: By creating a new VCS in which the central platform (i.e. the GitHub) is only an index akin to DNS. What I'd need from et al is a way to (1) for convenience: create accounts for those users and (2) allow restricted financial commitment by a 3rd party to the billing's account for a free tier.
