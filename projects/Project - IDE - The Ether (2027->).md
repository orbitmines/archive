...
The goal being that this is a physically plausible model. Or rather that's it's a step in the right direction. More likely it is some permutation of the ideas used here which yields 
...
`ether clone @c++`
`ether install @c++`
`ether use @c++` // Is like a checkout to a branch
`ether use --global @c++`
...
That way we can say the top-level `.ether/external/@/@clang` is the currently checked out version, with all the versions in the `nested @`. After a change is made, but switched to a diff version, we put it under `.ether/external/@/@clang/$/@<USER>%branch`

$ dir in a repository is all the difference branches.
`.ether/external/@/@clang/$/@windows/@x86-64/%/0.16.0`
...
User-controlled ownership of data: By creating a new VCS in which the central platform (i.e. the GitHub) is only an index akin to DNS. What I'd need from @googlecloud, @awscloud, @Azure et al is a way to:
(1) for convenience: create accounts for those users and 
(2) allow restricted financial commitment by a 3rd party to the billings account for a free tier.
...
mixed lang .js.ray where the .js is interpreted and the primitives of ray are underneath the ether package. Extension support for mixed syntax icon half .ray hald /.js for instance.
...
Video/Multimedia formats, just like databases. Optimizations for a very simply language interface: 2D/3D scenes + temporal, which need to be optimized for CPU time, storage, same considerations as the databases --> do you reformat, what do you need available. How does the optimizations come into play here (incentivizing it).
