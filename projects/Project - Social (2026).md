...
Specifically for now, as I said in 2023, I was hoping to by 2026 have a sample gamified problem. So I'm probably going to for the next two months, place a focus on that. The obvious candidate is an optimization problem: The problem is well-defined (not an open problem like much of coding), there's a definite equivalence to what kind of program we're looking for. Now the tricky part is figuring out how to map searching the space of possible programs to the lower-dimensional space that is a game UI. So that's going to be the main challenge, let's see if I can think of some good ideas in the next month or two. 
