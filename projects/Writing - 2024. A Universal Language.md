...
export class DynamicIterable<T> implements Iterable<T> {  
  
  private readonly refs: Iterable<T>[];  
  
  constructor(...args: T[]) { this.refs = args.length === 0 ? [] : [args] }  
  
  *[Symbol.iterator]() {  
    for (let ref of this.refs) { yield *ref; }  
  }  
...
class Ray implements Iterable<Ray> {  
  ...
  initial: DynamicIterable<Ray> = new DynamicIterable<Ray>();  
  self: DynamicIterable<any> = new DynamicIterable<any>();  
  terminal: DynamicIterable<Ray> = new DynamicIterable<Ray>();  
  
  constructor(...args: any[]) {  
    this.self.push(...args);  
  
    this.__proxy__ = new Proxy(Ray, {  
  ...
  *[Symbol.iterator](): Iterator<Ray> {  
  ...
  is_none = () => this.self.is_empty()  
  
}  
  
export default new Ray().__proxy__;
```