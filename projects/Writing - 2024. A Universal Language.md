...
Interpreting; Functions (and coordinate spaces), speeds, acceleration
...
Not traversal, but collapse of local connectivity to determine "next" steps. ; until current pointer holds something we're looking for. Always with intermediate results, which can be acted on separately: "what if this was the result" even when it's still pendi
