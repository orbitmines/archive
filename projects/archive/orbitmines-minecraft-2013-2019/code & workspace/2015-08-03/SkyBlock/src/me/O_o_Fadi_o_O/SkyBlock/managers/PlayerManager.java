...
import me.O_o_Fadi_o_O.SkyBlock.utils.Gadget;
...
public class PlayerManager {
	...
	public static boolean isOnOwnIsland(Player p, Location l2){
	...
	*/
	...
	public static void warnPlayerNotLoaded(Player p){
		...
		p.sendMessage("§5§lSkyBlock §8| §7This Server just §drestarted§7! Wait a few §dseconds§7.");
	}
	...
	*/
	...
	public void loadPlayer(final Player p){
		...
		try{	
			...
			if(DatabaseManager.containsPath("uuid", "Rank-VIP", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Rank-Staff", p.getUniqueId().toString())){
			...
			if(!DatabaseManager.containsPath("uuid", "Votes", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName().toLowerCase(), "Votes", "votes", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "OrbitMinesTokens", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "OrbitMinesTokens", "omt", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "VIPPoints", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "VIPPoints", "points", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "MiniGameCoins", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "MiniGameCoins", "coins", 0);
			}
			
			if(!DatabaseManager.containsPath("uuid", "MiniGameKills", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "MiniGameKills", "kills", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "MiniGameLoses", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "MiniGameLoses", "loses", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "MiniGameWins", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "MiniGameWins", "wins", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "SurvivalGames-Kills", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "SurvivalGames-Kills", "kills", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "SurvivalGames-Loses", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "SurvivalGames-Loses", "loses", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "SurvivalGames-Wins", p.getUniqueId().toString())){
				DatabaseManager.insertInt(p.getName(), "SurvivalGames-Wins", "wins", 0);
			}
			if(!DatabaseManager.containsPath("uuid", "Fireworks-Passes", p.getUniqueId().toString())){
			...
			/*
			if(DatabaseManager.containsPath("uuid", "Gadgets-Paintballs", p.getUniqueId().toString())){
				StorageManager.paintballsgadget.add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Gadgets-CreeperLauncher", p.getUniqueId().toString())){
				StorageManager.creeperlaunchergadget.add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Gadgets-PetRide", p.getUniqueId().toString())){
				StorageManager.petridegadget.add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Gadgets-BookExplosion", p.getUniqueId().toString())){
				StorageManager.bookexplosiongadget.add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Gadgets-SwapTeleporter", p.getUniqueId().toString())){
				StorageManager.swapteleportergadget.add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Gadgets-MagmaCubeSoccer", p.getUniqueId().toString())){
				StorageManager.magmacubesoccergadget.add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Gadgets-SnowmanAttack", p.getUniqueId().toString())){
				StorageManager.snowmanattackgadget.add(p);
			}
			*/
			
			boolean hasPet = false;
			if(DatabaseManager.containsPath("uuid", "Pets-MushroomCow", p.getUniqueId().toString())){
				StorageManager.mushroomcowpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Ocelot", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Pig", p.getUniqueId().toString())){
				StorageManager.pigpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Pig", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Wolf", p.getUniqueId().toString())){
				StorageManager.wolfpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Wolf", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Sheep", p.getUniqueId().toString())){
				StorageManager.sheeppet.put(p, DatabaseManager.getString(p.getName(), "Pets-Sheep", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Horse", p.getUniqueId().toString())){
				StorageManager.horsepet.put(p, DatabaseManager.getString(p.getName(), "Pets-Horse", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-MagmaCube", p.getUniqueId().toString())){
				StorageManager.magmacubepet.put(p, DatabaseManager.getString(p.getName(), "Pets-MagmaCube", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Slime", p.getUniqueId().toString())){
				StorageManager.slimepet.put(p, DatabaseManager.getString(p.getName(), "Pets-Slime", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Cow", p.getUniqueId().toString())){
				StorageManager.cowpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Cow", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Silverfish", p.getUniqueId().toString())){
				StorageManager.silverfishpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Silverfish", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Ocelot", p.getUniqueId().toString())){
				StorageManager.ocelotpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Ocelot", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Creeper", p.getUniqueId().toString())){
				StorageManager.creeperpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Creeper", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Squid", p.getUniqueId().toString())){
				StorageManager.squidpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Squid", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Spider", p.getUniqueId().toString())){
				StorageManager.spiderpet.put(p, DatabaseManager.getString(p.getName(), "Pets-Spider", "petname"));
				hasPet = true;
			}
			if(DatabaseManager.containsPath("uuid", "Pets-Chicken", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Wardrobe-Disco", p.getUniqueId().toString())){
			...
			*/
			...
			if(DatabaseManager.containsPath("uuid", "ChatColors-DarkRed", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.DARK_RED).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-LightGreen", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.LIGHT_GREEN).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-DarkGray", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.DARK_GRAY).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-Red", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.RED).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-White", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.WHITE).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-LightBlue", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.LIGHT_BLUE).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-Pink", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.PINK).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-Blue", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.BLUE).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-DarkBlue", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.DARK_BLUE).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-Green", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.GREEN).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "ChatColors-Black", p.getUniqueId().toString())){
				StorageManager.chatcolors.get(ChatColor.BLACK).add(p);
			}
			
			if(DatabaseManager.containsPath("uuid", "ChatColors-Bold", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "ChatColors-Cursive", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Trails-AngryVillager", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.ANGRY_VILLAGER).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Hearts", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.HEART).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Bubble", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.BUBBLE).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Crit", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.CRIT).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-ETable", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.ENCHANTMENT_TABLE).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Explode", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.TNT).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Firework", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.FIREWORK_SPARK).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-HappyVillager", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.HAPPY_VILLAGER).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-MobSpawner", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.MOB_SPAWNER).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Music", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.MUSIC).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Slime", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.SLIME).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Smoke", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.SMOKE).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Snow", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.SNOW).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Water", p.getUniqueId().toString())){
				StorageManager.trails.get(Trail.WATER).add(p);
				hasTrail = true;
			}
			if(DatabaseManager.containsPath("uuid", "Trails-Witch", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Trails-Type", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Trails-TypeBig", p.getUniqueId().toString())){
				StorageManager.trailtypes.get(TrailType.BIG_TRAIL).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Trails-TypeBody", p.getUniqueId().toString())){
				StorageManager.trailtypes.get(TrailType.BODY_TRAIL).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Trails-TypeGround", p.getUniqueId().toString())){
				StorageManager.trailtypes.get(TrailType.GROUND_TRAIL).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Trails-TypeHead", p.getUniqueId().toString())){
				StorageManager.trailtypes.get(TrailType.HEAD_TRAIL).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Trails-TypeVertical", p.getUniqueId().toString())){
				StorageManager.trailtypes.get(TrailType.VERTICAL_TRAIL).add(p);
			}
			
			if(DatabaseManager.containsPath("uuid", "Trails-TypeSpecial", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Trails-TypeParticlesAmount", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "ChatColors", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Hats-Bedrock", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.BEDROCK).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-BlackGlass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.BLACK_GLASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Cactus", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.CACTUS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-CoalBlock", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.COAL_BLOCK).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-CoalOre", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.COAL_ORE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Furnace", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.FURNACE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Glass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.GLASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Grass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.GRASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-GreenGlass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.GREEN_GLASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-HayBale", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.HAY_BALE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Ice", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.ICE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-LapisBlock", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.LAPIS_BLOCK).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-LapisOre", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.LAPIS_ORE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Leaves", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.LEAVES).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-MagentaGlass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.MAGENTA_GLASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Melon", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.MELON).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Mycelium", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.MYCELIUM).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-OrangeGlass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.ORANGE_GLASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-QuartzBlock", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.QUARTZ_BLOCK).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-QuartzOre", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.QUARTZ_ORE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-RedGlass", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.RED_GLASS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-RedstoneBlock", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.REDSTONE_BLOCK).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-RedstoneOre", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.REDSTONE_ORE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Snow", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.SNOW).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-StoneBricks", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.STONE_BRICKS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-TNT", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.TNT).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Workbench", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.WORKBENCH).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Diorite", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.DIORITE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-DarkPrismarine", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.DARK_PRISMARINE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Sponge", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.SPONGE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-SlimeBlock", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.SLIME_BLOCK).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-SeaLantern", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.SEA_LANTERN).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-PrismarineBricks", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.PRISMARINE_BRICKS).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Granite", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.GRANITE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Chest", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.CHEST).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Glowstone", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.GLOWSTONE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-WetSponge", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.WET_SPONGE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-Andesite", p.getUniqueId().toString())){
				StorageManager.hats.get(Hat.ANDESITE).add(p);
				hasHat = true;
			}
			if(DatabaseManager.containsPath("uuid", "Hats-BlueGlass", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Hats-BlockTrail", p.getUniqueId().toString())){
			...
			*/
			...
			if(DatabaseManager.containsPath("uuid", "Dis-Witch", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.WITCH).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Bat", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.BAT).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Chicken", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.CHICKEN).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Ocelot", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.OCELOT).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-MushroomCow", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.MUSHROOM_COW).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Squid", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SQUID).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Slime", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SLIME).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-ZombiePigman", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.ZOMBIE_PIGMAN).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-MagmaCube", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.MAGMA_CUBE).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Skeleton", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SKELETON).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Wolf", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.WOLF).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Spider", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SPIDER).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Silverfish", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SILVERFISH).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Sheep", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SHEEP).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-CaveSpider", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.CAVE_SPIDER).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Creeper", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.CREEPER).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Cow", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.COW).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Enderman", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.ENDERMAN).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Horse", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.HORSE).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-IronGolem", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.IRON_GOLEM).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Ghast", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.GHAST).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Snowman", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.SNOWMAN).add(p);
			}
			if(DatabaseManager.containsPath("uuid", "Dis-Rabbit", p.getUniqueId().toString())){
				StorageManager.disguises.get(Disguise.RABBIT).add(p);
			}
			
			/*
			
			if(DatabaseManager.containsPath("uuid", "ParkourCompleted", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "MasterMind-Wins", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "MasterMind-BestGame", p.getUniqueId().toString())){
				StorageManager.mindcraftbestgame.put(p, DatabaseManager.getInt(p.getName(), "MasterMind-BestGame", "turns"));
			}
			
			if(DatabaseManager.containsPath("uuid", "Hub-Players", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Hub-Stacker", p.getUniqueId().toString())){
			...
			if(DatabaseManager.containsPath("uuid", "Hub-Scoreboard", p.getUniqueId().toString())){
			...
			*/
			...
			if(DatabaseManager.containsPath("uuid", "PlayerNicknames", p.getUniqueId().toString())){
			...
			/*
			if(DatabaseManager.containsPath("uuid", "Fireworks-Settings", p.getUniqueId().toString())){
			...
			*/
			...
			TitleManager.setTitle(p, "§6§lOrbitMines§5§lSkyBlock", "");
			...
			new BukkitRunnable(){
				public void run(){
					...
					p.sendMessage(" ");
					ComponentMessageManager.sendComponentMessage(p, " §7§lSkyBlock Spawn Built By: ", "§e§l[View]", "show_text", "", "§b§lMod §bsharewoods\n§b§lMod §beekhoorn2000", "");
