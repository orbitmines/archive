...
import net.minecraft.server.v1_8_R1.BlockLog1;
...
public class DisguisePlayer {
	...
	public static void disguiseAsBlock(Player p, int id, Player... players) {
		...
		Block b = new BlockLog1();
		
		EntityFallingBlock disguise = new EntityFallingBlock(((CraftPlayer) p).getHandle().world, p.getLocation().getX(), p.getLocation().getY(), p.getLocation().getZ(), b.getBlockData());
        ...
        StorageManager.blockdisguise.put(p, id);
        
        for(Player player : players){
        	if(player != p){
        		if(StorageManager.playersenabled.get(player) == true){
	...
	public static void undisguisePlayer(Player p){
    	...
    	if(StorageManager.blockdisguise.containsKey(p)){
    		StorageManager.blockdisguise.remove(p);
    	}
