...
public class PlayerManager {
	...
	public static void setBossBar(Player p){
		if(bossbarnumber <= 10){
			...
			BossbarAPI.removeBar(p);
			BossbarAPI.setMessage(p, "§6§lOrbitMines§4§lNetwork §8| §6You have §6§l" + StorageManager.votes.get(Hub.getUUIDfromPlayer(p)) + "§6 Votes");
			bossbarnumber = 0;
		}
	}
	
	public static void setLastDonator(Player p){
		ConfigManager.playerdata.set("LastDonator", Hub.getUUIDfromPlayer(p).toString());
	...
	public void loadPlayer(final Player p){
		...
		try{
			if(DatabaseManager.containsPath("uuid", "PlayerSilentJoin", uuid)){
				boolean b = Boolean.parseBoolean(DatabaseManager.getString(p.getName(), "PlayerSilentJoin", "silentjoin"));
				StorageManager.silent.put(p, b);
			}
			...
			if(ConfigManager.playerdata.contains("players." + p.getName())){
				int votes = ConfigManager.playerdata.getInt("players." + p.getName() + ".Votes");
				ConfigManager.playerdata.set("players." + p.getName(), null);
				ConfigManager.playerdata.set("players." + uuid + ".Votes", votes);
				ConfigManager.savePlayerData();
			}
			
			if(!ConfigManager.playerdata.contains("players." + uuid + ".Votes")){
				ConfigManager.playerdata.set("players." + uuid + ".Votes", 0);
				if(StorageManager.allvoters.contains(uuid)){
					StorageManager.allvoters.remove(uuid);
				}
				StorageManager.allvoters.add(Hub.getUUIDfromPlayer(p));
			}
			ConfigManager.savePlayerData();
			...
			StorageManager.votes.put(Hub.getUUIDfromPlayer(p), DatabaseManager.getInt(p.getName(), "Votes", "votes"));
			ConfigManager.playerdata.set("players." + uuid + ".Votes", StorageManager.votes.get(Hub.getUUIDfromPlayer(p)));
			ConfigManager.savePlayerData();
			...
			for(final Player player : Bukkit.getOnlinePlayers()){
				if(StorageManager.playersenabled.containsKey(player)){
					if(StorageManager.playersenabled.get(player) == false){
						...
						if(StorageManager.blockdisguise.containsKey(player)){
			    			new BukkitRunnable(){
			    				public void run(){
			            			DisguisePlayer.disguiseAsBlock(player, StorageManager.blockdisguise.get(player), p);
			    				}
			    			}.runTaskLater(this.hub, 40);
						}
					}
			...
			TitleManager.setTitle(p, "§6§lOrbitMines§4§lNetwork", "");
			...
			new BukkitRunnable(){
				public void run(){
					...
					p.sendMessage(" ");
					ComponentMessageManager.sendComponentMessage(p, " §7§lHub Spawn Built By: ", "§e§l[View]", "show_text", "", "§b§lMod §bsharewoods\n§b§lMod §beekhoorn2000\n§d§lBuilder §drienk222\n§d§lBuilder §dcasidas\n§4§lOwner §4O_o_Fadi_o_O", "");
			...
			if(PlayerManager.hasStaffPerms(p, StaffRank.Builder)){
				Hub.permission.playerAddTransient(StorageManager.builderworld.getName(), p.getName(), "command.give.other");
				Hub.permission.playerAddTransient(StorageManager.builderworld.getName(), p.getName(), "command.skull");
				Hub.permission.playerAddTransient(StorageManager.builderworld.getName(), p.getName(), "command.fly");
				Hub.permission.playerAddTransient(StorageManager.builderworld.getName(), p.getName(), "command.gamemode");
				Hub.permission.playerAddTransient(StorageManager.builderworld.getName(), p.getName(), "worldedit.*");
			}
