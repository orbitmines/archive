...
public class HomeManager {
	...
	public static void loadHomes(Player p){
		...
		if(!ConfigManager.playerdata.contains("players." + uuid + ".MaxHomesAllowed")){
			ConfigManager.playerdata.set("players." + uuid + ".MaxHomesAllowed", 3);
			ConfigManager.savePlayerData();
			
			StorageManager.homesallowed.put(p, 3);
		}
		else{
			StorageManager.homesallowed.put(p, ConfigManager.playerdata.getInt("players." + uuid + ".MaxHomesAllowed"));
		}
		
		if(ConfigManager.playerdata.contains("players." + uuid + ".Homes")){
			List<String> homes = new ArrayList<String>();
			List<Location> homelocations = new ArrayList<Location>();
			for(String home : ConfigManager.playerdata.getConfigurationSection("players." + uuid + ".Homes").getKeys(false)){
				String[] location = ConfigManager.playerdata.getString("players." + uuid + ".Homes." + home).split("\\|");
	...
	public static void removeHome(Player p, String home){
		...
		if(StorageManager.homes.get(p).contains(home)){
			ConfigManager.playerdata.set("players." + uuid + ".Homes." + home, null);
			ConfigManager.savePlayerData();
	...
	public static void setHome(Player p, String home){
		...
		if(StorageManager.homesallowed.get(p) > StorageManager.homes.get(p).size() || StorageManager.homes.get(p).contains(home)){
			...
			ConfigManager.playerdata.set("players." + uuid + ".Homes." + home, location);
			ConfigManager.savePlayerData();
