...
public class NPCManager {
	...
	public void spawnArmorStand(World w, Location l, String displayname, boolean small, boolean arms, boolean baseplate, EulerAngle bodypose, boolean gravity, EulerAngle headpose, EulerAngle leftarmpose, EulerAngle leftlegpose, EulerAngle rightarmpose, EulerAngle rightlegpose, boolean visible, ItemStack helmet, ItemStack chestplate, ItemStack leggings, ItemStack boots, ItemStack hand, ArmorStandType type, int PlotNumber){
		...
		if(type == ArmorStandType.PLOT_EDITOR){
			StorageManager.ploteditornpc = as;
		}
		if(type == ArmorStandType.PLOT_KIT_SELECTOR){
			List<Entity> npcs = StorageManager.plotpvpnpcs.get(PlotNumber);
			npcs.add(as);
			StorageManager.plotpvpnpcs.put(PlotNumber, npcs);
		}
