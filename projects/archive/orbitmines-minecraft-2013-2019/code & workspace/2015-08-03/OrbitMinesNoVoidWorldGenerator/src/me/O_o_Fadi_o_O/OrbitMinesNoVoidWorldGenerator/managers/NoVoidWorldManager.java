package me.O_o_Fadi_o_O.OrbitMinesNoVoidWorldGenerator.managers;
...
public class NoVoidWorldManager extends ChunkGenerator {
	...
	public byte[][] generateNoVoidWorld(World w, Random r, int cX, int cY, int cZ, BiomeGrid bg){
		...
		for(x= 0; x < 16; x++){
			for(z = 0; z < 16; z++){
				setBlock(res, x, 92, z, (byte) 166);
				setBlock(res, x, 91, z, (byte) 49);
			}
		}
	...
	public short[][] generateExtBlockSections(World w, Random r, int xC, int zC, BiomeGrid biomes){
		...
		for(x= 0; x < 16; x++){
			for(z = 0; z < 16; z++){
				setBlock(res, x, 92, z, (short) 166);
				setBlock(res, x, 91, z, (short) 49);
			}
		}
