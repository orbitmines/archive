...
import me.O_o_Fadi_o_O.Creative.utils.Gadget;
...
public class PlayerManager {
	...
	public static boolean hasPlot(Player p){
		if(StorageManager.playerhasplot.containsKey(p)){
			if(StorageManager.playerhasplot.get(p) == true){
				return true;
			}
			else{
				return false;
	...
	private static Map<String, Long> lastMsg = new HashMap<String, Long>();
	private static final int msgcooldown = 2;
	
	public static boolean isOnOwnPlot(Player p, Location l2){
		if(l2.getBlockY() > 92){
			boolean onownplot = false;
			
			if(!StorageManager.playerspvpplot.containsKey(p)){
				if(PlayerManager.hasPlot(p)){
					Location l = StorageManager.plotlocation.get(StorageManager.playersplotnumber.get(p));
					int x = l.getBlockX();
					int z = l.getBlockZ();
			...
			if(onownplot == false){
				if(!StorageManager.playerspvpplot.containsKey(p)){
					if(StorageManager.playersplotmemberson.containsKey(p.getName())){
						for(String s : StorageManager.playersplotmemberson.get(p.getName())){
							if(onownplot == false){
					...
					if(StorageManager.plotpvpbuild.get(PlotNumber) == true){
			...
			if(onownplot == false){
				long lastMessage = 0;
				if(lastMsg.containsKey(p.getName())){
					lastMessage = lastMsg.get(p.getName());
				}
				int cdmillis2 = msgcooldown * 1000;
						
				if(System.currentTimeMillis()-lastMessage>=cdmillis2){
					p.sendMessage(Start.TAG + "§4§lDENIED§7 You cannot do that here!");
					
					lastMsg.put(p.getName(), System.currentTimeMillis());
				}
			}
	...
	*/
	...
	public static void warnPlayerNotLoaded(Player p){
		...
		p.sendMessage("§d§lCreative §8| §7This Server just §drestarted§7! Wait a few §dseconds§7.");
	}
	...
	public void loadPlayer(final Player p){
		...
		try{	
			...
			*/
			...
			TitleManager.setTitle(p, "§6§lOrbitMines§d§lCreative", "");
			
			new BukkitRunnable(){
				public void run(){
					p.sendMessage("§7§m----------------------------------------");
					p.sendMessage(" §6§lOrbitMines§4§lNetwork §7- §d§lCreative");
					p.sendMessage(" ");
					...
					p.sendMessage(" ");
					p.sendMessage(" §7§lCreative Spawn Built By: §d§lBuilder §drienk222");
