...
public class DisguisePlayer {
	...
	public static void disguisePlayer(Player p, EntityType type, boolean onfire, boolean adult, boolean customname, String name, Player... players) {
	    EntityLiving disguise = null;
	    switch(type) {
    	case BAT:
    		disguise = new EntityBat(((CraftPlayer) p).getHandle().world);
        ...
        if(disguise.getBukkitEntity() instanceof Ageable){
        	if(adult == true){
        		((Ageable) disguise.getBukkitEntity()).setAdult();
        	}
        	else{
        		((Ageable) disguise.getBukkitEntity()).setAdult();
        	}
        	((Ageable) disguise.getBukkitEntity()).setAgeLock(true);
        }
        
        disguise.fireTicks = Integer.MAX_VALUE;
        
        if(customname == true){
        	disguise.setCustomName(name);
        	disguise.getBukkitEntity().setCustomNameVisible(true);
        }
