...
public class InventoryClick implements Listener{
	...
	public void onClick(PlayerInteractEvent e){
		...
		if(!StorageManager.kitselected.containsKey(p)){
			if(lastUsage.containsKey(p.getName())){lastUsage.remove(p.getName());}
			if(lastUsage2.containsKey(p.getName())){lastUsage2.remove(p.getName());}
			if(lastUsage3.containsKey(p.getName())){lastUsage3.remove(p.getName());}
			if(lastUsage4.containsKey(p.getName())){lastUsage4.remove(p.getName());}
			if(lastUsage5.containsKey(p.getName())){lastUsage5.remove(p.getName());}
			if(lastUsage7.containsKey(p.getName())){lastUsage7.remove(p.getName());}
			if(lastUsage8.containsKey(p.getName())){lastUsage8.remove(p.getName());}
			if(lastUsage9.containsKey(p.getName())){lastUsage9.remove(p.getName());}
			if(lastUsage10.containsKey(p.getName())){lastUsage10.remove(p.getName());}
			if(lastUsage11.containsKey(p.getName())){lastUsage11.remove(p.getName());}
		}
					...
					if(h != null && h.getType() == Material.RAW_FISH && h.getItemMeta().getLore().contains("§7Fish Attack I")){
						...
						if(System.currentTimeMillis()-lastUsed8>=cdmillis8){
							...
							for(Player player : Bukkit.getOnlinePlayers()){
								...
								EnumParticle.WATER_SPLASH, true, (float) p.getLocation().getX(), (float) p.getLocation().getY(), (float) p.getLocation().getZ(), 2, 2, 2, 0, 50, null));	
							}
