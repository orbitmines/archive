	...
	public void onPlayerChatEvent(AsyncPlayerChatEvent e){
		...
		if(StorageManager.loaded.get(p) == true){
			...
			if(StorageManager.nickname.containsKey(p)){
				...
				e.setFormat("§7>§a§l" + PlayerManager.getRankString(p) + "§7< " + PlayerManager.getPlayerDisplayname(p).replaceAll(p.getName(), "*" + StorageManager.nickname.get(p) + color + "*") + "§7 » §" + StorageManager.chatcolor.get(p) + bold + cursive + "%2$s");
			}
			else{
				e.setFormat("§7>§a§l" + PlayerManager.getRankString(p) + "§7< " +PlayerManager.getPlayerDisplayname(p) + "§7 » §" + StorageManager.chatcolor.get(p) + bold + cursive + "%2$s");
			}
		}
		else{
			e.setCancelled(true);
