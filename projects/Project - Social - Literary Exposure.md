...
console.log([...document.querySelectorAll("#video-title")].filter(video => video.title !== '').forEach(video => {
	video.insertAdjacentHTML("afterend", policy.createHTML(`<div><input type="text" value="${video}" readonly hidden> <button onclick="this.previousElementSibling.select();document.execCommand('copy');">Copy</button></div>`));
}))
