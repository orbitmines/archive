...
- Possibilities
	- Operating system executables (pro: few of them, pretty much already acts as a cultural universal language, con: interpretability harder: reverse engineering necessary)
...
- Track and extract features.
	...
	- Extract APIs: the frontends/backends (+intermediates) -> Extract Grammars as Types. Extract Compilers between grammars as programs.
		- Extract features from grammars too: So changing input meaning a change in functionality, what downstream effects basically do changes in input cause? How do you clearcut make this a single "feature"
...
- MOP-like model, but we need some way of judging whether something is its own island in the sence of criticality -> is it hard to find this island?  What does this island allow us to find which other things don't. Energy is how many resources we have already dedicated to 
